 ...
 * Used ChatGPT to make the nice tree structure at the end but it didn't change code
...
int main(void) {
    ...
    set(DDRB, PB6); //set PB6 for the blue LED for the 200Hz
    clear(PORTB, PB1); //Make sure RED LED is off
    clear(PORTF, PF6); //Make sure that Green LED is off
    clear(PORTB, PB6); // make sure it is off in the beginning
    ...
    while (1) {
        ...
        if (currentState && !lastState) { // if there is a change in if the pin detects anything
            ...
            if (currenttime >= lasttime) { // if the new current time is more than the last time
                TimeDifference = currenttime - lasttime; // Calculate time difference in ticks by subtracting the last time from the current time
            }
            ...
            lasttime = currenttime; // set the current to last time for the next time you go through the sequence
            ...
            if (TimeDifference >= 9900 && TimeDifference <= 10100) { // Check if timeDiff is within the range of 25 Hz signal with a reasonable safety window 
                ...
                clear(PORTB, PB1); // otherwise don't turn on PB1
            }
            ...
            if (TimeDifference >= 375 && TimeDifference <= 381) { // check if the time difference between the ticks is inside the calculated ticks needed for the 662HZ signal with a reasonable safety window
                ...
                clear(PORTF, PF6); // otherwise turn off the LED
            }

            if (TimeDifference >= 1240 && TimeDifference <= 1260) { // Check for 200Hz signal
            ...
            /*
            m_usb_tx_string("Time Difference (Ticks): "); // sequence used for debugging prints the time difference to check if code is wrong or signal is wrong
        ...
        _delay_us(10); //small delay to prevent that my computer goes crazy
    }
